  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Court Calendar</h2>
          <p className="text-muted-foreground">View and manage hearing schedules</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-border bg-card">
            ...
            <div className="grid grid-cols-7">
              {emptyDays.map(i => (
                <div key={`empty-${i}`} className="min-h-[80px] border-b border-r border-border p-2" />
              ))}
              {days.map(day => {
                ...
                return (
                  <div
                    key={day}
                    className={cn(
                      "min-h-[80px] border-b border-r border-border p-2 transition-colors hover:bg-muted/30",
                  ...
                  >
                    ...
                    {dayCases.slice(0, 2).map((c, i) => (
                      <button
                        ...
                        className="mt-1 block w-full truncate rounded bg-accent/20 px-1.5 py-0.5 text-left text-xs font-medium text-accent-foreground hover:bg-accent/30"
                      >
                        {c.suitNumber}
                      </button>
                    ))}
        ...
        <div className="space-y-4">
            ...
            <div className="divide-y divide-border">
              {upcomingHearings.map((caseItem, index) => (
                <button
                  ...
                  className="w-full p-4 text-left transition-colors hover:bg-muted/30 animate-fade-in"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <div className="flex items-start gap-3">
                    <div className="flex h-10 w-10 flex-shrink-0 flex-col items-center justify-center rounded-lg bg-accent/10 text-accent-foreground">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {caseItem.court.split(' ').slice(0, 2).join(' ')}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {upcomingHearings.length === 0 && (
                <div className="p-6 text-center text-muted-foreground">
                  ...
                  <p className="mt-2">No upcoming hearings</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <h4 className="mb-3 text-sm font-medium text-foreground">Legend</h4>
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="h-3 w-3 rounded bg-accent/20" />
