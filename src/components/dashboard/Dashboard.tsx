...
export function Dashboard({
  ...
  // Calculate dynamic values for the cards
  const currentMonth = new Date().getMonth();
  const currentYear = new Date().getFullYear();

  const documentsThisMonth = auditLogs.filter((log) => {
    const logDate = new Date(log.timestamp);
    return (
      log.resource === "Document" &&
  ...
  const casesClosedThisMonth = cases.filter((c) => {
    ...
    // For now, count all closed cases since we don't have a closed date
    // This can be updated when closed date is added to the Case interface
    return true;
  }).length;
